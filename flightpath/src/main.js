...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'c' || e.key === 'C') {
    e.preventDefault();
    // Force refresh config to apply new line widths
    game.config = { ...makeScaledConfig(AIRSPACE_CONFIG), lineColors: AIRSPACE_CONFIG.lineColors };
    game.needsRedraw = true;
    game.showToast && game.showToast('Config refreshed - line widths updated');
    console.log('Config refreshed:', game.config.lineOutlineWidth, game.config.lineInnerWidth);
  }
  if (e.key === 't' || e.key === 'T') {
    e.preventDefault();
