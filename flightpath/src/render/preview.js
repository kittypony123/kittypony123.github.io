export function drawPreview(ctx, cam, preview){
  ...
  // Allow callers to specify preview color (defaults to existing palette)
  const baseColor = preview.color || 'rgba(14,165,163,0.9)';
  const invalidColor = preview.invalidColor || '#ff4444';
  const glowColor = preview.glowColor || (preview.color ? preview.color : 'rgba(14,165,163,0.4)');
  ...
  if (preview.snapStation !== undefined && preview.snapStation !== -1) {
    ...
    ctx.strokeStyle = preview.valid ? baseColor : invalidColor;
