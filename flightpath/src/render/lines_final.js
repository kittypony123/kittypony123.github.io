...
export function drawMultiStationLine(ctx, cam, game, line, overlapMap){
  ...
  // Waypoint markers at station connections (only for actual stations, not intermediate path points)
  drawStationMarkers(ctx, cam, line, game);
...
function drawStationMarkers(ctx, cam, line, game) {
  ctx.save();
  ctx.globalAlpha = 0.8;

  // Only draw markers at actual station positions, not intermediate waypoints
  for (let i = 0; i < line.stations.length; i++) {
    const station = game.stations[line.stations[i]];
    if (!station) continue;

    // Use actual station coordinates, not waypoint coordinates
    const point = { x: station.x, y: station.y };
    const radius = 4 / cam.scale;

    // Draw station connection marker
    ctx.beginPath();
