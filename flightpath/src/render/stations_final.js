export function drawStations(ctx, cam, game){
  ...
  for (let i = 0; i < game.stations.length; i++) {
    ...
    // Station circle with high-contrast border for dark backgrounds
    ctx.beginPath();
    ctx.fillStyle = '#ffffff';
    // Much thicker, high-contrast borders for visibility on dark backgrounds
    ctx.strokeStyle = s.isOvercrowded ? '#FF4444' : (isHovered ? '#40E0D0' : '#000000');
    ctx.lineWidth = (s.isOvercrowded ? 6 : (isHovered ? 5 : 4)) / cam.scale;
