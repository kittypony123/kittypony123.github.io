...
export function spawnPassenger(game){
  ...
  // SMART DESTINATION SELECTION - consider route connectivity
  function hasRouteConnection(fromId, toId) {
    return game.lines.some(line =>
      line.stations.includes(fromId) && line.stations.includes(toId)
    );
  }

  function getConnectedDestinations(fromId, destinations) {
    if (!destinations || destinations.length === 0) return [];

    const connected = destinations.filter(dest => hasRouteConnection(fromId, dest.id));

    // 80% prefer connected, 20% allow unconnected (for growth pressure)
    if (connected.length > 0 && Math.random() < 0.8) {
      return connected;
    }
    return destinations; // Always return something valid
  }
  ...
  if (finals.length > 0) {
    ...
    if (Math.random() < finalBias || others.length === 0) {
      const connectedFinals = getConnectedDestinations(originIndex, finals);
      if (connectedFinals.length > 0) {
        destStation = connectedFinals[Math.floor(Math.random() * connectedFinals.length)];
      } else {
        // Fallback to any final if no connected ones
      ...
      if (pickHub) {
        const connectedHubs = getConnectedDestinations(originIndex, interchanges);
        if (connectedHubs.length > 0) {
          destStation = connectedHubs[Math.floor(Math.random() * connectedHubs.length)];
        } else {
          destStation = interchanges[Math.floor(Math.random() * interchanges.length)];
        }
      } else {
        const connectedOthers = getConnectedDestinations(originIndex, others);
        if (connectedOthers.length > 0) {
          destStation = connectedOthers[Math.floor(Math.random() * connectedOthers.length)];
        } else {
