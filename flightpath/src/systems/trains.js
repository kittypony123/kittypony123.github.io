...
export function createTrain(game, lineId){
  ...
  // Randomize starting position and direction to prevent trains following each other
  const existingTrains = line.trains.length;
  const startingPosition = existingTrains > 0 ? Math.random() : 0.0; // First train starts at 0, others random
  const startingDirection = Math.random() > 0.5 ? 1 : -1; // Random direction
  ...
  // Apply route-specific speed based on route type
...
export function handleStationArrival(game, train, station){
  ...
  for (const p of train.passengers){
    if (p.destStation === station.id){
      ...
      const waitTime = now - p.spawnTime;
      game.updateRouteMetrics(train.lineId, true, waitTime);
...
// Dynamic train allocation based on line length and passenger demand (CONSERVATIVE)
export function optimizeTrainAllocation(game) {
  if (game.trainsAvailable <= 0) return;

  // Calculate desired trains per line based on length and demand (more conservative)
  const lineStats = game.lines.map(line => {
    ...
    // CONSERVATIVE: Base trains needed - 1 train per 500+ units of length
    const baseTrainsForLength = Math.max(1, Math.ceil(length / 500));

    // CONSERVATIVE: Additional trains only for very high passenger demand  
    const demandTrains = Math.ceil(passengerDemand / 15); // 1 extra train per 15 passengers

    // CONSERVATIVE: Station count factor - fewer extra trains
    const stationTrains = Math.max(1, Math.ceil(stationCount / 4));

    // CONSERVATIVE: Lower maximum trains to prevent over-allocation
    const maxTrains = Math.min(3, Math.max(1, Math.ceil(stationCount / 3)));

    // CONSERVATIVE: Prioritize lines with zero trains only
    let desiredRaw = Math.max(baseTrainsForLength, stationTrains) + Math.min(1, demandTrains);
    if ((game.config && game.config.hubAndSpokeMode) && connectsHub) {
      const bonus = Math.min(1, (game.config.hubDesiredTrainBonus != null) ? game.config.hubDesiredTrainBonus : 1);
      desiredRaw += bonus;
    }
    ...
    return {
      ...
      priority: zeroTrainBoost + length + (passengerDemand * 30) + (connectsHub ? (game.config.hubLinePriorityBonus || 100) : 0)
    };
  });

  // Sort by priority (zero trains first, then longest lines with highest demand)
  lineStats.sort((a, b) => b.priority - a.priority);

  // CONSERVATIVE: Only allocate to highest priority line per cycle
  const topLine = lineStats[0];
  if (topLine && topLine.deficit > 0) {
    const newTrain = createTrain(game, topLine.line.id);
    if (newTrain) {
      game.trainsAvailable--;
      console.log(`Added aircraft to line ${topLine.line.id} (${topLine.currentTrains + 1}/${topLine.desiredTrains} aircraft)`);
    }
  }
}
