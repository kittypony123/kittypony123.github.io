...
export function normalizeLineColors(game){
...
export function createLine(game, stations, colorIndex=null){
  ...
  if (colorIndex !== null && colorIndex !== undefined && colorIndex >= 0) {
    ...
    // Use strategic color based on route type if available
    if (game.getStrategicColor && stations.length >= 2) {
      const strategicColor = game.getStrategicColor(stations[0], stations[stations.length - 1]);
      finalColorIndex = strategicColor;
    } else {
  ...
  const line = {
    ...
    get color(){
      const index = this.colorIndex >= 0 && this.colorIndex < game.config.lineColors.length
        ? this.colorIndex
        : this.colorIndex % game.config.lineColors.length;
      return game.config.lineColors[index] || '#FFB347'; // fallback to orange
    },
  ...
  // FIXED: Don't trigger train allocation immediately - let the manual creation in input handle it
  // The input system creates one train manually, optimization will handle additional ones later
...
export function rebuildWaypointsForLine(game, line){
  clearHexCache(); // Clear cache to force recalculation with new logic
  ...
  for (let i=0; i<line.stations.length-1; i++){
    ...
    const B = game.stations[line.stations[i+1]];
    if (!A || !B) continue;
    let seg = createHexPath(A.x, A.y, B.x, B.y, game.config.hexGrid.size, game);
