...
export function attachInput(canvas, game, onPreview){
  ...
  // Tap-to-confirm route helpers
  const TAP_MAX_DISTANCE = 8; // px
  const TAP_MAX_TIME = 250; // ms
  let pointerDownScreenX = 0; let pointerDownScreenY = 0; let pointerDownTs = 0;
  let routeDragActive = false;
  let pendingTap = null; // { stationIdx: number }
  ...
  let touchStartX=0; let touchStartY=0; // Track initial touch position

  // Detect if device primarily uses touch to avoid double event handling
  // Use more reliable detection based on actual mobile device patterns
  const isTouchDevice = (function() {
    // Primary check: mobile user agent patterns
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
      return true;
    }
    // Secondary check: Windows/MacOS touch devices (Surface, touchscreen laptops)
    // But exclude desktop browsers that just support touch APIs
    if (navigator.maxTouchPoints > 1) {
      // Additional checks to avoid false positives on desktop Chrome
      const hasCoarsePointing = window.matchMedia && window.matchMedia('(pointer: coarse)').matches;
      const isMobileLike = window.screen.width <= 1024 || hasCoarsePointing;
      return isMobileLike;
    }
    return false;
  })();
  ...
  // Cache canvas rect for performance (recalculate on resize)
  let cachedRect = null;
  let lastRectUpdate = 0;
  function rect(){
    const now = performance.now();
    if (!cachedRect || now - lastRectUpdate > 100) { // Cache for 100ms
  ...
  // Helper to copy touch properties (iOS Safari reuses touch objects)
  function copyTouchProps(touch) {
    return {
  ...
  function selectedPreviewColor(){
    const colors = game.config.lineColors || [];
    if (!colors.length) return 'rgba(14,165,163,0.9)';
    const idx = (game.selectedLineColorIndex != null && game.selectedLineColorIndex < colors.length && game.selectedLineColorIndex >= 0)
      ? game.selectedLineColorIndex
      : 0;
  ...
  document.addEventListener('keydown', (e)=>{
    ...
    if (e.key==='Escape'){ onPreview(null); resetPendingTap(); }
  ...
  // Always use pointer events but handle touch conversion separately
  canvas.addEventListener('pointerdown', (ev)=>{
    // On touch devices, only handle synthetic events; on non-touch devices, handle native events
    if (DEBUG) {
      console.log('pointerdown:', {
        isTouchDevice,
        synthetic: ev.synthetic,
        pointerType: ev.pointerType,
        willProcess: !(isTouchDevice && !ev.synthetic) && !(!isTouchDevice && ev.synthetic)
      });
    }
    if (isTouchDevice && !ev.synthetic) return;
    if (!isTouchDevice && ev.synthetic) return;
    isDown=true;
    ...
    try {
      ...
      // Ignore capture errors on touch devices
    }
    const r = rect();
    ...
    pointerDownScreenX = screenX;
    pointerDownScreenY = screenY;
    ...
    if (isDown && !routeDragActive) {
      const movement = Math.hypot(screenX - pointerDownScreenX, screenY - pointerDownScreenY);
      if (movement > TAP_MAX_DISTANCE) {
    ...
    if (dragStartStation!=null){
      ...
      onPreview({ points, valid: true, snapStation: endIdx, isHexSnapped: true, color: selectedPreviewColor() });
      ...
      onPreview({ points, valid, snapStation: endIdx, isHexSnapped: true, color: selectedPreviewColor() });
    }

    // Pending tap preview (hover after first tap)
    if (!isDown && pendingTap != null) {
      const start = game.stations[pendingTap.stationIdx];
      if (start) {
        const endIdx = game.hoveredStationIdx;
        const end = (endIdx !== -1 ? game.stations[endIdx] : world);
        let points = createHexPath(start.x, start.y, end.x, end.y, game.config.hexGrid.size, game);
        ...
        points = applyCorridorBundling(points, game, null);
        let valid = true;
        if (endIdx === pendingTap.stationIdx) {
          valid = false;
        } else if (endIdx !== -1) {
          const crosses = segmentCrossesPolygon({ x: start.x, y: start.y }, { x: end.x, y: end.y }, game.config.thamesPolygon);
  ...
  canvas.addEventListener('pointerup', (ev)=>{
    ...
    if (isTouchDevice && !ev.synthetic) return;
    if (!isTouchDevice && ev.synthetic) return;
    isDown=false; isPanning=false; routeDragActive = false;
    try{
    ...
    const elapsed = performance.now() - pointerDownTs;
    const releaseMove = Math.hypot(screenX - pointerDownScreenX, screenY - pointerDownScreenY);
    const isTap = releaseMove <= TAP_MAX_DISTANCE && elapsed <= TAP_MAX_TIME;

    if (isTap && dragStartStation!=null){
      if (pendingTap == null){
        pendingTap = { stationIdx: dragStartStation };
        dragStartStation=null;
        game.needsRedraw = true;
        return;
      }
      if (pendingTap.stationIdx === dragStartStation){
        resetPendingTap();
        dragStartStation=null;
        return;
      }
      const startIdx = pendingTap.stationIdx;
      pendingTap = null;
      dragStartStation = startIdx;
    } else if (isTap && pendingTap != null && hit===-1){
      resetPendingTap();
    }
  ...
  canvas.addEventListener('touchstart', (ev) => {
    ...
    if (ev.touches.length === 2) {
      ...
      const touch = isIOSSafari ? copyTouchProps(ev.touches[0]) : ev.touches[0];
      ...
      pointerEvent.synthetic = true; // Mark as converted from touch
  ...
  canvas.addEventListener('touchend', (ev) => {
    ...
    if (ev.touches.length === 0) {
      ...
      if (isGesturing) {
        ...
        // Quick tap - check if movement was minimal to confirm it's a tap
        const touch = isIOSSafari ? copyTouchProps(ev.changedTouches[0]) : ev.changedTouches[0];
        const deltaX = Math.abs(touch.clientX - touchStartX);
        const deltaY = Math.abs(touch.clientY - touchStartY);

        if (deltaX > TOUCH_TOLERANCE || deltaY > TOUCH_TOLERANCE) {
          // Too much movement, treat as drag end instead of tap
          let pointerEvent;
          try {
        ...
        // Mark both events as synthetic
        pointerDown.synthetic = true;
        pointerUp.synthetic = true;
