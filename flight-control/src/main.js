...
const game = {
  config: { ...AIRSPACE_CONFIG, lineColors: AIRSPACE_CONFIG.lineColors },
  camera: null,
  stations: [],
  lines: [],
  trains: [], // planes
...
function loop(ts){
  try {
    ...
    if (game.needsRedraw || hasUpdates) {
      ...
      let alert=false; const thresh=40; // world units
      ...
      if (wrap && toast){
        if (alert){
          ...
          toast.textContent='Traffic alert — increase spacing';
          toast.style.opacity='1';
          toast.style.transform='translateX(-50%) translateY(0)';
          clearTimeout(loop._toastTimer);
          loop._toastTimer = setTimeout(()=>{ try{ toast.style.opacity='0'; toast.style.transform='translateX(-50%) translateY(-8px)'; }catch(e){} }, 1200);
...
// Auto-routing button and keyboard shortcut
document.getElementById('btnAutoRoute').addEventListener('click', ()=> {
  if (game.autoRouting) game.autoRouting.toggleAutoRouting();
});
