...
export function attachInput(canvas, game, onPreview){
  ...
  canvas.addEventListener('pointerdown', (ev)=>{
    isDown=true; canvas.setPointerCapture(ev.pointerId); const world = worldOf(ev);
  ...
  canvas.addEventListener('pointermove', (ev)=>{
    ...
    if (dragStartStation!=null){
      const start=game.stations[dragStartStation]; const endIdx = game.hoveredStationIdx; const end = (endIdx!==-1? game.stations[endIdx] : world); const points = createHexPath(start.x, start.y, end.x, end.y, game.config.hexGrid.size); onPreview({ points, valid: true, snapStation: endIdx }); if (DEBUG) console.log('preview from', dragStartStation, 'to', endIdx);
      ...
      const points = createHexPath(start.x, start.y, end.x, end.y, game.config.hexGrid.size);
      ...
      onPreview({ points, valid, snapStation: endIdx });
    }
  });
  canvas.addEventListener('pointerup', (ev)=>{
    isDown=false; try{ canvas.releasePointerCapture(ev.pointerId);}catch(e){}
