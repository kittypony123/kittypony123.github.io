...
export class AutoRoutingSystem {
  constructor(game) {
    this.game = game;
    this.enabled = false;
    this.complexityThreshold = 8; // Enable when complexity score > 8 (lowered for easier testing)
    this.lastAutoAction = 0;
    this.actionCooldown = 8000; // 8 seconds between auto actions
    this.suggestions = [];
  ...
  update() {
    ...
    // Check if we should enable/disable auto-routing
    const shouldEnable = this.shouldEnableAutoRouting();

    if (shouldEnable && !this.enabled) {
      this.enabled = true;
      const complexity = this.calculateComplexity();
      if (this.game.debugLogs) console.log(`Auto-routing ENABLED: Complexity ${complexity} >= ${this.complexityThreshold}`);
      ...
      this.enabled = false;
      const complexity = this.calculateComplexity();
      if (this.game.debugLogs) console.log(`Auto-routing DISABLED: Complexity ${complexity} < ${this.complexityThreshold}`);
  ...
  performAutoActions() {
    const actions = [
      ...
      () => this.autoCreateFeederLines(),
      () => this.autoOptimizeUnderperformingLines()
    ];
  ...
  toggleAutoRouting() {
    ...
    const complexity = this.calculateComplexity();
    if (this.game.debugLogs) console.log(`Manual auto-routing toggle: ${status} (Complexity: ${complexity}/${this.complexityThreshold})`);
